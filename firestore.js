...
const FS_GLOBAL = ['__fil__', '__usr__', '__pc__', '__app__'];
...
const FS = {
  ...
  async loadGlobal() {
    ...
    FS_CACHE.setGlobal('__fil__', docs[0].exists ? docs[0].data().lista || [] : []);
  ...
  async loadFilial(filialId) {
    ...
    const db = FB.db;
    const reads = FS_COLLECTIONS.map(col =>
      db.doc(fsPath(filialId, col)).get().then(snap => ({ col, data: snap.exists ? snap.data().lista || [] : [] }))
    );
  ...
  async set(filialId, col, data) {
    ...
    if (!FB.ready || !FB.db) return; // só cache local se offline
